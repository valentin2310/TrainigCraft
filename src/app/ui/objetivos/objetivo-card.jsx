...
export default function ObjetivoCard({ objetivo }) {
    ...
    return (
        <>
        <div className={clsx(
            "w-full h-full bg-gradient-to-tl from-gray-50 to-gray-100 text-secondary p-2 xl:p-4 rounded-xl shadow flex justify-between gap-3",
            {
            ...
            <div className="flex gap-1 xl:gap-5">
                ...
                <div className="">
                    <div className="flex gap-2">
                        <p className={`xl:text-xl line-clamp-1 xl:line-clamp-2 ${dificultadColor(objetivo.dificultad)}`}>{objetivo.titulo}</p>
                    ...
                    <p className="text-sm xl:text-md line-clamp-2 text-secondary/90">{objetivo.descripcion ? objetivo.descripcion : 'Sin descripción.'}</p>
                </div>
            </div>
            <CardEditDots 
                onOpen={onOpen}
